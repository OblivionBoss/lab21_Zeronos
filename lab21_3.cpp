...
int main(){
	...
	while(getline(student_file,textline)){
		...
		int locf = textline.find_first_of(',');
		s.name = textline.substr(0,locf);
		s.id = atof(textline.substr(locf+1,4).c_str());
		s.gender = textline.substr(locf+6,1);
		int locl = textline.find_last_of(',');
		s.gpa = atof(textline.substr(locl+1,4).c_str());
